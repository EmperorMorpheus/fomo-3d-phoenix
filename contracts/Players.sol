...
interface IPlayersBook {
    ...
    function addTotalReferralEarnings(address playerAddress, uint earnings) external view returns(uint256);
...
contract PlayersBook {
    ...
    function registerPlayerInfo(uint256 refCode, address payable affAddress, address payable playerAddress, uint256 currentRound) external onlyAllowedContract(){
        ...
        players[playerAddress] = Player(refCode, affiliateAddress, new string[](0), 0, 0, 0, currentRound, 0);
   ...
   function getPlayer(address playerAddress) public view returns (Player memory) {
    ...
    function addTotalReferralEarnings(address playerAddress, uint earnings) public onlyAllowedContract returns(uint256) {
        require(earnings > 0, "no referral earnings to add for player balance is 0");
        uint256 totalEarnings = players[playerAddress].referralEarnings += earnings;
        return totalEarnings;
    }

    function addClaimedDividends(address playerAddress, uint256 playerDivies) public onlyAllowedContract {
        require(playerDivies > 0, "no dividends to add for player");
